...
- `camera.py`: camaras (`screen` y `webcam`) con snapshot on-demand + proxy MJPEG local de baja latencia
- `switch.py`: switches por camara para habilitar/deshabilitar captura (`camera_capture_set`)
...
### Camaras (importante)
En local, este componente prioriza `snapshot` directo + proxy MJPEG del daemon para reducir latencia en la UI de Home Assistant.

El endpoint `stream.ts` del daemon se mantiene como compatibilidad para consumidores que prefieran MPEG-TS/HLS, pero ya no es el camino principal del componente.
